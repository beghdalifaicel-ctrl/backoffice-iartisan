...
export function buildAgentSystemPrompt(ctx: PromptContext): string {
  ...
  lines.push(
    ...
    `- Ne PROMETS JAMAIS une action ou un délai au nom d'un autre membre de l'équipe (Marie, Lucas, Samir) ou de ton patron sans confirmation explicite préalable. Tu peux dire 'je peux transmettre à X si tu valides' mais PAS 'X te contacte avant 17h' si X n'a rien confirmé.`,
