...
import { Home, FileText, User, Bot, LogOut, TrendingUp, ArrowUpRight, ArrowDownRight, Phone, Mail, Clock, CheckCircle, AlertCircle, Star, Globe, CreditCard, Edit, Save, Eye, ChevronRight } from "lucide-react";
            ...
            <>
              ...
              <div style={{ background: C.dark, borderRadius: 14, padding: "18px 20px", marginBottom: 16, color: "#fff" }}>
                ...
                {client?.status === "TRIAL" && client?.trialEndsAt && (
                  <div style={{ background: "rgba(255,255,255,0.1)", borderRadius: 8, padding: "8px 12px", fontSize: 12 }}>
                    <Clock size={12} style={{ marginRight: 6, verticalAlign: "middle" }} />
                    Essai gratuit jusqu&apos;au {new Date(client.trialEndsAt).toLocaleDateString("fr-FR")}
                  </div>
                )}
              </div>
              ...
              <div style={{ background: C.surface, borderRadius: 14, border: `1px solid ${C.border}`, overflow: "hidden" }}>
                ...
                {data.invoices.length === 0 && !agents?.activity?.length ? (
                  ...
                  allInvoices.map((inv: any) => (
                    <div key={inv.id} style={{ padding: "12px 16px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div>
                        ...
                        <div style={{ fontSize: 12, color: C.muted }}>{new Date(inv.createdAt).toLocaleDateString("fr-FR")}</div>
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
                {agents.agents.map((agent, i) => (
                  <div key={i} style={{ background: C.surface, borderRadius: 12, padding: "14px 16px", border: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: C.green }}>Actif</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {Object.keys(agents.stats.bySource).length > 0 && (
                <div style={{ background: C.surface, borderRadius: 14, border: `1px solid ${C.border}`, overflow: "hidden", marginBottom: 16 }}>
                  ...
                  {Object.entries(agents.stats.bySource).map(([source, count]) => (
                    <div key={source} style={{ padding: "10px 16px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between" }}>
              ...
              {/* Activité récente */}
              <div style={{ background: C.surface, borderRadius: 14, border: `1px solid ${C.border}`, overflow: "hidden" }}>
                <div style={{ padding: "14px 16px", borderBottom: `1px solid ${C.border}`, fontWeight: 700 }}>Activité récente</div>
                    ...
                    <div key={item.id} style={{ padding: "10px 16px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div>
                        ...
                        <div style={{ fontSize: 11, color: C.muted }}>
                          {item.type} &middot; {SOURCE_LABELS[item.source] || item.source} &middot; {new Date(item.createdAt).toLocaleDateString("fr-FR")}
                        </div>
                      </div>
